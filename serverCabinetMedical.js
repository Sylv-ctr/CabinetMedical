...
function init(port, applicationServerIP, applicationServerPort) {
	...
	// Define HTTP ressource POST /addPatient, may contains new patient information
	app.post( '/addPatient'
			, function(req, res) {
    ...
    // Define HTTP ressource POST /removePatient, may contains numeroSecu patient information
    app.post( '/delPatient'
			, function(req, res) {
				 console.log("/delPatient, \nreq.body:\n\t", req.body, "\n_______________________");
                //requiert le numero de sécu du patient
				 var patientNumber = req.body.patientNumber;
				 
        
            var patients = doc.getElementsByTagName('patient');
            var patientTrouve = false;
        
            for(var i = 0 ; i < patients.length ; i++ ){
                var monPatient = patients[i];
                var numero = monPatient.getElementsByTagName("numero")[0].textContent;
                if(numero==patientNumber){
                    doc.getElementsByTagName("patients")[0].removeChild(monPatient);
                    patientTrouve = true;
                    break;
    ...
    app.post( '/affectation'
	      , function(req, res) {
        console.log("traitement affectation");
    ...
    app.post('/desaffecter'
            , function (req, res) {

                //console.log("traitement désaffectation");
                console.log("/desaffecter, \nreq.body:\n\t", req.body.patient, "\n_______________________");

                // le numéro du patient qu'on recherche
                var patientNumberData = req.body.patient;
                //console.log(patientNumberData);

                // récuperer tous les patients du document
                var patients = doc.getElementsByTagName("patient");

                // rechercher le patient ayant le même numero de sécurité sociale que le paramètre
                var patientTrouve = false;
                for (var i = 0; i < patients.length; i++) {

                    var monPatient = patients[i];
                    var numeroPatient = monPatient.getElementsByTagName("numero")[0].textContent;

                    // puis masquer les visites du DOM
                    if (numeroPatient === patientNumberData) {
                        monPatient.getElementsByTagName('visite')[0].removeAttribute('intervenant');
   ...
   app.post( '/INFIRMIERE'
	       , function(req, res) {
                ...
                //recupere la liste des infirmiers dans le fichier xml (doc)
				 var infirmiers =  doc.getElementsByTagName('infirmier'), id;
                for(var i=0; i<infirmiers.length; i++) {
			      id = infirmiers[i].getAttribute('id');
                    console.error(id);
                    console.error(req.body.id);
                    //test si l'id de notre infirmier correspond au paramètre
                    if(id==req.body.id){
                        trouve = true;
                        break;
                    }
                ...
                //si un infirmier match avec le parametre alors on se connecte et affiche le fichier html interfaceInfi
                 if(trouve){
                     fs.readFile( __dirname + '/interfaceInfi.html',
							  function (err, data) {
								if (err) {
									 res.writeHead(500);
									 return res.end('Error loading secretary.html : ', err);
                     ...
                     console.error("Identifiant incorrect");
                 }
